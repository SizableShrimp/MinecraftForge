...
//    }
    ...
    public List<Path> getMinecraftPaths() {
        var mcversion = FMLLoader.versionInfo().mcVersion()+"-"+FMLLoader.versionInfo().mcpVersion();
        var mc = LibraryFinder.findPathForMaven("net.minecraft", "client", "", "srg", mcversion);
        var mcextra = LibraryFinder.findPathForMaven("net.minecraft", "client", "", "extra", mcversion);
        return List.of(mc, mcextra);
    }
}
