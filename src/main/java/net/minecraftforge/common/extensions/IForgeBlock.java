...
public interface IForgeBlock
{
     ...
     */
    default float getSlipperiness(BlockState state, LevelReader world, BlockPos pos, @Nullable Entity entity)
    {
     ...
     */
    default int getLightValue(BlockState state, BlockGetter world, BlockPos pos)
    {
    ...
    default BlockEntity createTileEntity(BlockState state, BlockGetter world)
    {
        if (getBlock() instanceof EntityBlock)
            return ((EntityBlock)getBlock()).newBlockEntity(world);
        return null;
    }
     ...
     */
    default Optional<Vec3> getBedSpawnPosition(EntityType<?> entityType, BlockState state, LevelReader world, BlockPos pos, float orientation, @Nullable LivingEntity sleeper)
    {
    ...
    */
     ...
     */
    default boolean isAir(BlockState state, BlockGetter world, BlockPos pos)
    {
     ...
     */
    default boolean canBeReplacedByLeaves(BlockState state, LevelReader world, BlockPos pos)
    {
     ...
     * @return true if this block can be replaced by growing leaves.
     */
    default boolean canBeReplacedByLogs(BlockState state, LevelReader world, BlockPos pos)
    {
    ...
    */
     ...
     */
    @OnlyIn(Dist.CLIENT)
    default boolean addHitEffects(BlockState state, Level world, HitResult target, ParticleEngine manager)
    {
        return false;
    }
    ...
    */
    ...
    default void onPlantGrow(BlockState state, LevelAccessor world, BlockPos pos, BlockPos source)
    {
    ...
    */
   @Deprecated//TODO - 1.17: Remove (unused)
    default void observedNeighborChange(BlockState observerState, Level world, BlockPos observerPos, Block changedBlock, BlockPos changedBlockPos){}
    ...
    */
    ...
    default float[] getBeaconColorMultiplier(BlockState state, LevelReader world, BlockPos pos, BlockPos beaconPos)
    {
        if (getBlock() instanceof BeaconBeamBlock)
            return ((BeaconBeamBlock)getBlock()).getColor().getTextureDiffuseColors();
        return null;
    }
     ...
     */
    @OnlyIn(Dist.CLIENT)
    default Vec3 getFogColor(BlockState state, LevelReader world, BlockPos pos, Entity entity, Vec3 originalColor, float partialTicks)
    {
        if (state.getMaterial() == Material.WATER)
        {
            ...
            return new Vec3(0.02F + f12, 0.02F + f12, 0.2F + f12);
        }
        else if (state.getMaterial() == Material.LAVA)
        {
            return new Vec3(0.6F, 0.1F, 0.0F);
        }
    ...
    default boolean canEntityDestroy(BlockState state, BlockGetter world, BlockPos pos, Entity entity)
    {
        ...
        {
            return state.isAir(world, pos) || WitherBoss.canDestroy(state);
        }
