                ...
                .then(Commands.literal("te")
                    .then(Commands.argument("duration", IntegerArgumentType.integer(1))
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new TranslatableComponent("commands.forge.tracking.te.enabled", duration), true);
                            return 0;
                        })
                    )
                )
        ...
        static ArgumentBuilder<CommandSourceStack, ?> register()
        {
            return Commands.literal("reset")
                ...
                .then(Commands.literal("te")
                    .executes(ctx -> {
                        TimeTracker.TILE_ENTITY_UPDATE.reset();
                        ctx.getSource().sendSuccess(new TranslatableComponent("commands.forge.tracking.te.reset"), true);
                        return 0;
                    })
                )
                .then(Commands.literal("entity")
